-- lua/hoverfloat/symbol_prefetcher.lua - FIXED VERSION (No cursor movement!)
local M = {}
...
local prefetch_state = {
  ...
  config = {
    ...
    cache_ttl_ms = 45000,
    -- No debouncing - let natural concurrency limiting handle rate control
  }
}
...
-- Create LSP position params for a specific line/column WITHOUT moving cursor
local function make_position_params_for_position(bufnr, line, col)
  ...
  return {
    ...
    position = {
      line = line - 1,  -- Convert to 0-based indexing
...
-- Background LSP request functions (NO CURSOR MOVEMENT!)
local function get_hover_info_at_position(bufnr, line, col, callback)
  if not has_capability('hoverProvider') then
    callback(nil)
    return
  end

  local params = make_position_params_for_position(bufnr, line, col)
...
local function get_definition_info_at_position(bufnr, line, col, callback)
...
local function get_references_info_at_position(bufnr, line, col, callback, max_refs)
...
local function get_type_definition_info_at_position(bufnr, line, col, callback)
...
-- Prefetch LSP data for a specific symbol (BACKGROUND ONLY - NO CURSOR MOVEMENT!)
local function prefetch_symbol_lsp_data(bufnr, symbol, callback)
  ...
  -- Use symbol's start position for LSP requests
  local target_line = symbol.start_line
  local target_col = symbol.start_col
  
  -- Collect LSP data WITHOUT moving cursor
  local lsp_data = {}
  ...
  -- Make all LSP requests at the symbol position (NO CURSOR MOVEMENT!)
  get_hover_info_at_position(bufnr, target_line, target_col, function(data) 
    complete_request("hover", data) 
  end)
  get_definition_info_at_position(bufnr, target_line, target_col, function(data) 
    complete_request("definition", data) 
  end)
  get_references_info_at_position(bufnr, target_line, target_col, function(data) 
    complete_request("references", data) 
  end, 8)
  get_type_definition_info_at_position(bufnr, target_line, target_col, function(data) 
    complete_request("type_definition", data) 
  end)
end
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    callback = function()
      ...
      get_document_symbols(bufnr, function(symbols)
        ...
        -- Start prefetching immediately after symbols are loaded
        vim.defer_fn(function()
          queue_visible_symbols_for_prefetch(bufnr)
        end, 100) -- Just enough delay to let buffer finish loading
      end)
    end,
  })
  
  -- Prefetch on scroll/window movement - immediate response
  vim.api.nvim_create_autocmd({ "WinScrolled" }, {
    group = group,
    callback = function()
      -- Immediate prefetching - no debouncing
      local bufnr = vim.api.nvim_get_current_buf()
      queue_visible_symbols_for_prefetch(bufnr)
    end,
  })
  ...
  vim.api.nvim_create_autocmd({ "TextChanged", "TextChangedI" }, {
    ...
    callback = function()
      ...
      get_document_symbols(bufnr, function(symbols)
        ...
        -- Start prefetching new symbols immediately
        queue_visible_symbols_for_prefetch(bufnr)
      end)
    end,
  })
